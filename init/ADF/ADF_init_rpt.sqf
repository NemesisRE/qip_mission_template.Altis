...
if (mod_TFAR) then {ADF_log_rptMods = ADF_log_rptMods + ", TFAR";};
if (mod_CTAB) then {ADF_log_rptMods = ADF_log_rptMods + ", cTab";};
if (mod_ACE3) then {ADF_log_rptMods = ADF_log_rptMods + ", ACE3";};
if (mod_AIA) then {ADF_log_rptMods = ADF_log_rptMods + ", AiATP";};
...
if (ADF_Log_ServerPerfEnable) then {
	[] spawn {
		waitUntil {
			_ADF_rptSnooz = 60;
